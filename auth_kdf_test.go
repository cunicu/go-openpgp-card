...
func TestSetupKDF(t *testing.T) {
	withCard(t, true, func(t *testing.T, c *opc.Card) {
		...
		if c.Capabilities.Flags&opc.CapKDF == 0 {
			t.Skip("Card does not support key derivation")
		}

		err := c.SetupKDF(opc.AlgKDFIterSaltedS2K, 100000, opc.DefaultPW1, opc.DefaultPW3)
		require.NoError(err)
		...
		// Remove KDF again
		err = c.SetupKDF(opc.AlgKDFNone, 0, opc.DefaultPW1, opc.DefaultPW3)
		require.NoError(err)
...
func TestKDF(t *testing.T) {
	...
	k := &opc.KDF{
		Algorithm:     opc.AlgKDFIterSaltedS2K,
		HashAlgorithm: opc.AlgHashSHA256,
		Iterations:    100000,
	...
	hashDefaultPW1, err := k.DerivePassword(opc.PW1, opc.DefaultPW1)
	require.NoError(err)
	require.Equal(expectedHashDefaultPW1, hashDefaultPW1)

	hashDefaultPW3, err := k.DerivePassword(opc.PW3, opc.DefaultPW3)
	require.NoError(err)
